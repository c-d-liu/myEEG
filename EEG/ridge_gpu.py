# GPU backend for ridge_cv_stratified_group.
# Only imported when USE_GPU is True, so cuML/rmm are only pulled in when actually needed.
import numpy as np
...
def fit_stratified_group_cv(X, y, blocks, groups, alphas, n_splits=5, random_state=None,
    ...
    """GPU (cuML) implementation of the stratified-group ridge CV fit.
    Mirrors ridge_cv_backend_cpu.fit_stratified_group_cv exactly in inputs/outputs
    (may return CuPy arrays; the caller/dispatcher is responsible for moving results
    back to host with cv_helpers.to_cpu).
    ...
    if optimize_alpha:
        for alpha in alphas:
            ...
            for train_idx, test_idx in stratified_group_kfold(blocks, groups,
                ...
                with rmm.statistics.profiler(name="model_fit"):
        ...
        best_alpha_per_channel = alphas[best_alpha_indices]
